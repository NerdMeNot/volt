//! Bench — spawn-hot with **single wait** per batch (matches Go's wg.Wait shape).
//!
//! Same as bench-spawn-hot but waits via an atomic-counter + Notify
//! pattern instead of 1000 individual task.join calls. This matches the
//! shape of Go's `wg := sync.WaitGroup{}; wg.Add(N); go ...; wg.Wait()`.
//!
//! Tasks still need cleanup (task.join frees Frame + Task), but those
//! joins are fast-path because the barrier already saw all `.done`'s.
//!
//! Use this when comparing against Go side-by-side — the original
//! bench-spawn-hot's 1000 individual joins is not what Go users write.
...
pub fn main() !void {
    ...
    std.debug.print("=== spawn+wait_all (Notify barrier, Go wg.Wait shape) ===\n", .{});
