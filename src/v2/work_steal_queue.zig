...
pub const InjectionQueue = @import("worker.zig").InjectionQueue;
...
pub const WorkStealQueue = struct {
    ...
    /// into a stealing region).
    pub fn push(self: *Self, coro: *Coroutine, global: *InjectionQueue) void {
        ...
        while (true) {
            ...
            while (i < half) : (i += 1) {
                ...
                if (self.buffer[idx].load(.acquire)) |t| global.push(t);
            }
            self.buffer[tail & MASK].store(coro, .release);
            self.tail.store(tail +% 1, .release);
            return;
...
test "WorkStealQueue: single-threaded push/pop FIFO" {
    ...
    var inj = worker_mod.InjectionQueue{};
