...
//!
//! Why not pthread_cond? POC-D (2026-05-12) measured ~600 ns/wake of
//! pthread bookkeeping overhead on Darwin. The underlying primitive
//! is `__ulock_wait` / `__ulock_wake` (also what `os_unfair_lock`
//! uses internally). Going direct shaves ~250 ns / cycle and drops a
//! dependency on libpthread internals.
