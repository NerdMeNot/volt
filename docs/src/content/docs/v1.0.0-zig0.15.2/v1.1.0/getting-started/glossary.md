...
sidebar:
  order: 4
slug: v1.0.0-zig0.15.2/v110/getting-started/glossary
---
...
**Local Queue**
A per-worker 256-slot lock-free ring buffer ([Chase-Lev deque](/v1.0.0-zig0.15.2/algorithms/chase-lev-deque/)) holding pending tasks. The owner thread has fast, uncontested access; other threads can steal from it.
...
**Work Stealing**
When a worker runs out of local tasks, it steals half the queue from a randomly chosen victim worker. See [Work Stealing](/v1.0.0-zig0.15.2/algorithms/work-stealing/).
...
**Channel(T)**
Bounded MPMC (multi-producer, multi-consumer) channel backed by a lock-free [Vyukov ring buffer](/v1.0.0-zig0.15.2/algorithms/vyukov-mpmc/). Requires an allocator and `deinit()`.
...
**Blocking Operation**
An operation that blocks the OS thread (not just the task). On a Volt worker thread, blocking stalls every task on that worker. Examples: `std.Thread.sleep`, `volt.net.resolve()`, `volt.fs.readFile()`. See [Common Pitfalls](/v1.0.0-zig0.15.2/guides/common-pitfalls/#operations-that-block-the-thread).
