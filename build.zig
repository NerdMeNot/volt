...
pub fn build(b: *std.Build) void {
    ...
    const benches = [_]struct { name: []const u8, src: []const u8, desc: []const u8 }{
        ...
        .{ .name = "repro-sigpipe", .src = "bench/repro_sigpipe.zig", .desc = "Regression: Runtime.init ignores SIGPIPE (write to peer-closed socket returns EPIPE, not process death)" },
    };
    for (benches) |bench| {
