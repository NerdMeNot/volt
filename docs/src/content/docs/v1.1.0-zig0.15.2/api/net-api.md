...
Every `tryRead`, `tryAccept`, and `tryRecv` returns four possible outcomes: data, would-block (`null`), peer-close (`0`), and error. Handling only two causes bugs. See [Common Pitfalls](/guides/common-pitfalls/#the-three-way-io-pattern) for the copy-paste template.
:::
