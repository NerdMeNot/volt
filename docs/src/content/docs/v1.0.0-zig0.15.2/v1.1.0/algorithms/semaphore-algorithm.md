...
description: The batch semaphore algorithm with direct-handoff and lock-free
  fast waiter slot used in Volt, designed to prevent permit starvation under
  contention.
slug: v1.0.0-zig0.15.2/v110/algorithms/semaphore-algorithm
---
...
The `fast_waiter` field is an atomic pointer that stores at most one waiter for lock-free release. See [Fast Waiter Slot](/v1.0.0-zig0.15.2/design/fast-waiter-slot/) for the full design rationale and why this is fundamentally different from the `has_waiters` flag that deadlocked.
...
* Fast waiter slot design: [Fast Waiter Slot](/v1.0.0-zig0.15.2/design/fast-waiter-slot/)
