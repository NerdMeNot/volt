//! Bench — Spsc channel send+recv pairs across two coroutines.
//!
//! Reference numbers:
//!   POC-F (two threads, busy-spin): 29 ns/op
//!   Go chan cap=16:                 33 ns/op
//!
//! Single-worker uses park/unpark when the channel fills or
...
const Channel = volt.Spsc(u64, 16);
...
pub fn main() !void {
    ...
    std.debug.print("  Spsc send+recv: {d} ns/op (median, min {d}, max {d})\n", .{ median, min, max });
    std.debug.print("\nReference:\n", .{});
