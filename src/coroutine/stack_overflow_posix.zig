//! POSIX (Darwin / Linux / BSD) implementation of the stack-overflow
//! handler. See `stack_overflow.zig` for the cross-platform dispatcher
//! and contract.
//!
//! Strategy:
//!   - Process-wide SIGSEGV/SIGBUS handler (sigaction).
//!   - Per-thread sigaltstack so the handler runs even when the
//!     coroutine's own stack is exhausted.
//!   - Per-dispatch sigsetjmp checkpoint; siglongjmp on terminal
//!     overflow back to the scheduler.
...
test "stack_overflow_posix: SigJmpBuf is large enough" {
