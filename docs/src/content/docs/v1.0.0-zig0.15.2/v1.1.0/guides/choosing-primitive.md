...
description: Decision tree for selecting the right synchronization primitive or
  channel type in Volt.
slug: v1.0.0-zig0.15.2/v110/guides/choosing-primitive
---
...
`volt.Group` is not a sync primitive -- it's a structured concurrency tool for spawning and managing multiple tasks. Use it when you need to spawn N tasks and wait for all to complete (or cancel all). See [Basic Concepts](/v1.0.0-zig0.15.2/getting-started/basic-concepts/#structured-concurrency-with-groups).
:::
...
Based on benchmark results (M3 Mac, 8 workers). Note: these numbers use 8 worker threads for contended benchmarks. The [Tokio comparison](/v1.0.0-zig0.15.2/testing/comparing-with-tokio/) uses 4 workers to match Tokio's default configuration, so numbers there will differ.
