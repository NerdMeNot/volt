...
description: Understand the fundamental programming model of Volt -- async/await, the runtime, Groups, and cooperative scheduling.
sidebar:
  order: 3
---
...
For the full waker lifecycle, internal API, and detailed diagrams, see [The Future Model](/design/future-model/).
...
For how Volt relates to Zig's `std.Io` (in development, expected in 0.16) and the differences between the two approaches, see [Stackless vs Stackful](/design/stackless-vs-stackful/).
