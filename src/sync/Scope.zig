...
pub const Scope = struct {
    ...
    children: std.array_list.Managed(*Job),
    ...
    fn cancelAllInternal(self: *Scope) void {
        for (self.children.items) |j| j.cancel();
    }
    ...
    fn joinAllInternal(self: *Scope) void {
        ...
        for (self.children.items) |j| j.join() catch {};
    }

    fn deinit(self: *Scope) void {
        for (self.children.items) |j| destroyJob(j);
...
pub fn scope(comptime body: anytype) !void {
    ...
    var s = Scope{
        ...
        .children = std.array_list.Managed(*Job).init(rt.allocator),
    };
    defer s.deinit();
