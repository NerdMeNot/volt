...
//!
//! Wide save set: 14 callee-saves (x19–x28, fp, lr, sp, d8–d15).
//! Narrow saves that omit NEON registers don't measurably reduce
//! latency on M-series, so the wide set wins on correctness — any
//! caller (SIMD-using or not) is safe across a swap.
//!
...
/// already zeroed** — the caller (`Runtime.spawn`) zeroes it via the
/// Coroutine struct's default initializer, so re-zeroing here would
/// duplicate a ~160-byte write per spawn. Pool-recycled Coroutines
/// also pass through `c.* = .{...}` which resets ctx to default.
